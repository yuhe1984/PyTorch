...
plt.figure(125)
plt.scatter(test_x.numpy(), test_y.numpy(), c='#9359FF', s=50, alpha=0.2, label='test')
plt.legend(loc='upper left')
...
# print(*nets)    # print net arecitecture
...
def plot_histogram(l_in, l_in_bn, pre_ac, pre_ac_bn):
    for i, (ax_pa, ax_pa_bn, ax, ax_bn) in enumerate(zip(axs[0, :], axs[1, :], axs[2, :], axs[3, :])):
        ...
        ax.hist(l_in[i].data.numpy().ravel(), bins=10,range=the_range, color='#FF9359', alpha=0.5)
        ax_bn.hist(l_in_bn[i].data.numpy().ravel(), bins=10,range=the_range, color='#74BCFF')
        for a in [ax_pa, ax_pa_bn, ax, ax_bn]: a.set_yticks(());a.set_xticks(())
        ax_pa_bn.set_yticks(p_range);ax_bn.set_xticks(the_range)
